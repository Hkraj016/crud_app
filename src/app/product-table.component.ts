...
export class ProductTableComponent {
  ...
  // Function to handle 'Enter' and 'Space' key presses on checkbox
  ...
  filterProducts() {
    ...
    this.filteredProducts = this.products.filter(product =>
      product.title.toLowerCase().includes(filterTextLower) ||
      product.category.toLowerCase().includes(filterTextLower) ||
      product.price.toString().includes(filterTextLower) ||
      product.stock.toString().includes(filterTextLower)
    );
  ...
  resetSelection() {
    ...
    this.filteredProducts.forEach(product => product.selected = false); // Reset selection
  }
  ...
  get selectedCount() {
    return this.filteredProducts.filter(product => product.selected).length; // Count of selected products
  }
